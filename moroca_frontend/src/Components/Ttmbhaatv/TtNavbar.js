...
//  
  const handleClick=(id)=>{
    setIsDropdownOpen(false);
      ...
      navigate(`/ttcategory/${id}/${service}`)
   }

   const fetchWatchlist = async () => {
    try {
      const response = await axios.get(`${backend_url}/api/tt/getTTWatchlist/${service}`);
  ...
  mt-2 space-y-2 w-48 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-700 rounded-lg shadow-lg`}  >
    {category.map((cat) => (
      ...
      >
        ...
        <li className='dropdown-container'>
          <a  class={`block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0  md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent ${service === 'raceday' ? 'hover:text-[#f73b3b]' : 'hover:text-blue-700'}`}
...
{isWatchlistDropdownOpen && (
  <div className=" absolute mt-2  w-50 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-700 rounded-lg shadow-lg "> 
    {watchlist.map((video) => (
  <Link to={`/ttsingle/${video.id}/${service}`}>
      <a
        key={video.videoid}
